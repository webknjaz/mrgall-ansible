...
class Connection(ConnectionBase):
    ...
    def _spawn_ssh_agent(self):
        self._ssh_agent = subprocess.Popen(['ssh-agent', '-sD'], stdout=subprocess.PIPE)
    ...
    def _sshpass_available():
        ...
        if SSHPASS_AVAILABLE is None:
            try:
                p = subprocess.Popen(["sshpass"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=self._get_subprocess_env())
                p.communicate()
    ...
    def _bare_run(self, cmd, in_data, sudoable=True, checkrc=True):
        ...
        if not in_data:
            try:
                ...
                if PY3 and self._play_context.password:
                    ...
                    p = subprocess.Popen(cmd, stdin=slave, stdout=subprocess.PIPE, stderr=subprocess.PIPE, pass_fds=self.sshpass_pipe, env=self._get_subprocess_env())
                else:
                    p = subprocess.Popen(cmd, stdin=slave, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=self._get_subprocess_env())
        ...
        if not p:
            if PY3 and self._play_context.password:
                # pylint: disable=unexpected-keyword-arg
                p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, pass_fds=self.sshpass_pipe, env=self._get_subprocess_env())
            else:
                p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=self._get_subprocess_env())
            stdin = p.stdin
